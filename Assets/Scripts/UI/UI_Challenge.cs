...
public class UI_Challenge : MonoBehaviour
{
    ...
    [Header("Progress")]
    [SerializeField] ChallengeCellDisplay[] challengeCellDisplays;
    public void UpdateProgress() //this is garbage
    {
        foreach (ChallengeCellDisplay i in challengeCellDisplays)
            i.UpdateProgress();
    }
    ...
    public void UpdateHoopPassed(int count)
    {
        hoopPassed.text = count + "/" + ChallengeManager.Instance.lastLevel.totalHoops+" hoops";
        descriptionPause.text = "Hoops completed " + count + "/" + ChallengeManager.Instance.lastLevel.totalHoops;
    }
}
